  ...
  return <div className="mt-9 space-y-6">
    <section className="card p-6" id="datasets">
      <div className="flex flex-wrap justify-between gap-5 items-start">
        <div className="upload-heading"><div className="eyebrow">Step 1 · Upload</div><h2 className="text-2xl font-semibold mt-2">Bring your IQVIA workbook</h2><p className="text-sm text-[var(--muted)] mt-1">The file stays in your browser while its table is inspected.</p></div>
        <label className="upload-button btn-primary inline-flex items-center gap-2">{busy ? "Working…" : "Choose workbook"}<input className="hidden" type="file" accept=".xlsx,.xls" disabled={busy} onChange={(e) => e.target.files?.[0] && inspect(e.target.files[0])}/></label>
      </div>
    ...
    <section className="card p-6"><div className="flex justify-between items-center"><div><div className="eyebrow">Recent datasets</div><h2 className="text-xl font-semibold mt-1">Analysis workspace</h2></div><span className="text-xs text-[var(--muted)]">{history.length} total</span></div><div className="mt-4 divide-y divide-[var(--line)]">{history.slice(0,8).map(item => <div key={item.id} className="dataset-row py-3 flex flex-wrap justify-between gap-3"><div><div className="font-semibold text-sm">{item.filename}</div><div className="text-xs text-[var(--muted)] mt-1">{item.row_count.toLocaleString()} rows · {item.column_count} columns</div></div><div className="dataset-actions flex items-center gap-2"><span className="status-pill status-good">{item.status}</span>{["normalised","analysed"].includes(item.status)&&<button className="btn-secondary text-xs" onClick={()=>setSelectedDataset(item)}>Open</button>}<button className="btn-secondary text-xs text-red-700" onClick={async()=>{if(!window.confirm(`Delete ${item.filename} and all derived analysis?`))return;try{await deleteDataset(item.id);setHistory(await listDatasets());if(selectedDataset?.id===item.id)setSelectedDataset(undefined);}catch(e){setMessage(e instanceof Error?e.message:"Delete failed");}}}>Delete</button></div></div>)}</div></section>
