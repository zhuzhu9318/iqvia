...
import { loadSetup, saveCategories, savePortfolio, type SetupCategory, type SetupIngredient, type SetupProduct } from "@/lib/data/normalise";
...
export function SetupWorkflow({ datasetId, onComplete }: { datasetId: string; onComplete: () => void }) {
  ...
  useEffect(() => { loadSetup(datasetId).then((data) => { setProducts(data.products); setIngredients(data.ingredients); setCategories(data.categories); const own = data.products.find((p) => p.is_portfolio)?.corporation; if (own) setCompany(own); }).catch((e) => setMessage(e.message)).finally(() => setBusy(false)); }, [datasetId]);
  async function persist() {
    ...
    setBusy(true);
    try { await savePortfolio(datasetId, company); await saveCategories(datasetId, categories, ingredients); setMessage("Portfolio and category definitions saved."); onComplete(); }
  ...
  return <section className="card" id="categorisation">
    ...
    <div className="grid lg:grid-cols-[.8fr_1.2fr]">
      <div className="p-6 border-r border-[var(--line)]"><label className="text-xs font-bold uppercase tracking-wider text-[var(--muted)]">This is our company</label><select className="input mt-2" value={company} onChange={(e) => setCompany(e.target.value)}><option value="">Select corporation…</option>{corporations.map((name) => <option key={name}>{name}</option>)}</select><div className="mt-5 text-sm"><span className="font-semibold">{products.filter((p) => p.corporation === company).length}</span> products will be marked as portfolio.</div><div className="mt-4 max-h-52 overflow-auto space-y-2">{products.filter((p) => p.corporation === company).slice(0,30).map((p) => <div key={p.id} className="rounded-lg bg-[#f3f6f4] px-3 py-2 text-xs">{p.name}</div>)}</div></div>
      <div className="p-6"><div className="flex justify-between gap-3"><label className="text-xs font-bold uppercase tracking-wider text-[var(--muted)]">Ingredient categories</label><span className="text-xs text-[var(--muted)]">{ingredients.length} ingredients</span></div><div className="mt-4 grid gap-4 sm:grid-cols-2">{categories.map((category, categoryIndex) => <div key={category.id} className="rounded-xl border border-[var(--line)] p-4"><input className="input font-semibold" value={category.name} onChange={(e) => setCategories((current) => current.map((item, index) => index === categoryIndex ? {...item, name: e.target.value} : item))}/><div className="mt-3 space-y-2">{ingredients.filter((item) => item.categoryId === category.id).map((ingredient) => <div key={ingredient.id} className="rounded-lg bg-[var(--mint)] p-2"><div className="text-xs font-semibold">{ingredient.name}</div><select className="mt-1 w-full bg-transparent text-[11px] text-[var(--muted)]" value={ingredient.categoryId ?? ""} onChange={(e) => setIngredients((current) => current.map((item) => item.id === ingredient.id ? {...item, categoryId: e.target.value} : item))}>{categories.map((target) => <option key={target.id} value={target.id}>{target.name}</option>)}</select></div>)}</div></div>)}</div>
